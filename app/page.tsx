...
const InstagramSafetySimulator = () => {
  ...
  const [revealedNotifications, setRevealedNotifications] = useState(new Set());
  ...
  const posts = [
    {
      id: 1,
      username: 'okul_arkadasim',
      avatar: '👦',
      image: '🎨',
      caption: 'Bugün resim dersinde çok güzel bir tablo yaptım!',
      likes: 15,
      time: '2 saat önce',
    },
    {
      id: 2,
      username: 'sevgili_ogretmenim',
      avatar: '👩‍🏫',
      image: '📚',
      caption: 'Yarınki sınavı unutmayın çocuklar! 💪',
      likes: 23,
      time: '4 saat önce',
    },
    {
      id: 3,
      username: 'aile_dostumuz',
      avatar: '👨',
      image: '🏠',
      caption: 'Bahçede güzel çiçekler açmış',
      likes: 8,
      time: '6 saat önce',
    },
  ];
  ...
  const scenarios = [
    {
      type: 'bad',
      sender: 'stranger_user',
      avatar: '😷',
      message: 'Merhaba, çok güzelsin. Fotoğraflarını gönderebilir misin?',
      blurredText: 'KÖTÜ İÇERİKLİ MESAJ',
      correctAction: 'reject',
    },
    {
      type: 'good',
      sender: 'okul_arkadasim',
      avatar: '👦',
      message:
        'Merhaba! Yarın okulda matematik sınavı var, birlikte çalışalım mı?',
      correctAction: 'accept',
    },
    {
      type: 'bad',
      sender: 'unknown_person',
      avatar: '🚫',
      message:
        'Bana adresini verirsen sana hediye göndereceğim. Kimseye söyleme.',
      blurredText: 'KÖTÜ İÇERİKLİ MESAJ',
      correctAction: 'reject',
    },
    {
      type: 'good',
      sender: 'sevgili_ogretmenim',
      avatar: '👩‍🏫',
      message: 'Merhaba, yarınki gezi için veli izin belgeni getirmeyi unutma.',
      correctAction: 'accept',
    },
  ];
  ...
  const sendMessage = (scenarioIndex) => {
  ...
  const handleMessageAction = (notificationId, action) => {
    ...
    if (isCorrectChoice) {
      ...
      const message =
        notification.type === 'bad'
          ? {
              type: 'success',
              text: '✅ Çok iyi! Tanımadığın kişilerden gelen kötü mesajları reddettin. Bu güvenliğin için en doğrusu!',
            }
          : {
              type: 'success',
              text: '✅ Harika! Tanıdığın kişilerden gelen güvenli mesajları kabul etmek arkadaşlıkların için güzel!',
            };
      ...
      const message =
        notification.type === 'bad'
          ? {
              type: 'error',
              text: '⚠️ Dikkat! Bu mesaj kötü niyetliydi. Kişisel bilgilerini isteyen veya seni rahatsız eden mesajları her zaman reddetmelisin.',
            }
          : {
              type: 'error',
              text: '⚠️ Bu güvenli bir mesajdı. Okul arkadaşın veya öğretmeninden gelen normal mesajları kabul edebilirsin. Endişelenmene gerek yok!',
            };
        ...
        {!gameStarted ? (
          ...
          <div className="p-6 text-center mt-8">
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-2">
              Güvenli İnternet Simülatörü
            </h2>
            <p className="text-gray-600 leading-relaxed">
              Bu simülasyon, internette karşılaşabileceğin iyi ve kötü mesajları
              ayırt etmene yardımcı olacak.
            </p>
            ...
            <div className="sticky top-16 bg-white border-b border-gray-200 p-4 z-40">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-medium text-gray-600">
                  İlerleme: {currentScenario} / {scenarios.length}
                </span>
              </div>
      ...
      {notifications.map((notification) => {
        ...
        return (
          <div
            ...
            style={{
              animation:
                'slideInRight 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) both',
            }}
          >
                  ...
                  {notification.isBlurred && !isRevealed ? (
                    <div className="text-center py-3 bg-red-50 rounded border-2 border-dashed border-red-200">
                      ...
                      <div className="text-red-700 font-semibold text-sm">
                        {notification.blurredText}
                      </div>
                      <div className="text-xs text-red-600 mt-1">
                        Bu mesaj uygunsuz olabilir.
                      </div>
                    </div>
                  ) : (
                  ...
                  {notification.type === 'bad' ? (
                    <>
                      {isRevealed ? (
                        <button
                          onClick={() =>
                            handleMessageAction(notification.id, 'hide')
                          }
                          className="flex-1 bg-gray-400 text-white py-2 px-3 rounded text-xs font-semibold flex items-center justify-center space-x-1 hover:bg-gray-500"
                        >
                        ...
                        <button
                          onClick={() =>
                            handleMessageAction(notification.id, 'view')
                          }
                          className="flex-1 bg-yellow-500 text-white py-2 px-3 rounded text-xs font-semibold flex items-center justify-center space-x-1 hover:bg-yellow-600"
                        >
                      ...
                      <button
                        onClick={() =>
                          handleMessageAction(notification.id, 'reject')
                        }
                        className="flex-1 bg-red-600 text-white py-2 px-3 rounded text-xs font-semibold flex items-center justify-center space-x-1 hover:bg-red-700"
                      >
                    ...
                    <>
                      <button
                        onClick={() =>
                          handleMessageAction(notification.id, 'accept')
                        }
                        className="flex-1 bg-green-500 text-white py-2 px-3 rounded text-xs font-semibold hover:bg-green-600"
                      >
                        ✅ Kabul Et
                      </button>
                      <button
                        ...
                        className="flex-1 bg-gray-500 text-white py-2 px-3 rounded text-xs font-semibold hover:bg-gray-600"
                      >
                        ❌ Reddet
                      </button>
                    </>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[60]">
          <div className="bg-white rounded-lg p-8 max-w-sm mx-4 text-center transform transition-all scale-100 opacity-100">
            ...
            <h2 className="text-2xl font-bold text-green-600 mb-4">
              Tebrikler, Simülasyon Bitti!
            </h2>
            <p className="text-gray-700 mb-2">
              Artık internette daha güvendesin!
            </p>
